...
export default class RoundService {
  ...
  static async exportRoundTicketsToCSV(roundId: number) {
    ...
    const roundTicketsSet = new Set(
      roundTicketsResult[0]?.map((row: any) => row.ticket_number) || []
    )
  ...
  static async startRound(params: StartParams) {
    ...
    //Busca tickets do banco secondary
    ...
    const validatedTickets = validatedTicketsResult[0] || []

    //Insere no banco principal
    let ticketsCount = 0
    if (validatedTickets.length > 0) {
      const placeholders = validatedTickets
        .map(() => '(?, ?, ?, ?, 1, NULL, NOW(), NOW())')
        .join(',')
      const bindings: any[] = []

      validatedTickets.forEach((ticket: any) => {
        bindings.push(round.raffleId, roundId, ticket.id, ticket.ticket_number)
      })
      ...
      ticketsCount = result[0]?.affectedRows || 0
    }
  ...
  static async checkTicketHits(params: checkTicketHitsParams) {
    ...
    const placeholders = hitTickets
      .map(() => '(?, ?, ?, ?, ?, 1, ?, NOW(), 0, NULL, NOW(), NOW())')
      .join(',')
    const bindings: any[] = []
